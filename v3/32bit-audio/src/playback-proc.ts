type ProcessFunction = (rsul: ReadableStreamReadResult<Uint8Array>) => ReadableStreamReadDoneResult<Uint8Array>;
const frame = 36;
const chunk = 1024;
/* @ts-ignore */
class PlaybackProcessor /* @ts-ignore */ extends AudioWorkletProcessor {
	...
	handleMesg(evt: MessageEvent) {
		...
		reader
			.read()
			.then(function process(this: any, { done, value }): any {
				if (done) {
			...
			.then((result: ReadableStreamReadDoneResult<Uint8Array>) => {
				process;
			});
	}
	report() {
