...
type AudioInputMaster = {
	ctx: AudioContext;
	crossFades: CrossFade[]; //does not talk about cross fades
...
export const initInputModule = async (): Promise<AudioInputMaster> => {
	/* consent = user having triggeredany of the follow: mouseEvent, keyboardEvent, etc*/
	/* @param withUserConsent is a boolean here because its not needed */
	/*  when Chrome flag is launched with flag --autoplay-policy=no-user-gesture-require
			and is set to false in the unit-test environment */
	...
	function miscComponents({ upload, sidechain, sharedBuffer }): Partial<AudioInputMaster | void> {
		const ctx = getCtx();
	...
	staticInputMaster = await getContext()
		...
		.then(miscComponents)
		.then((inputModule: Partial<AudioInputMaster>) => {
			const {
				merger,
				analyzer,
				ctx,
