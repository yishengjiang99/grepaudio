...
function processKernel() {
  ...
  for (let i = 0; i < CONFIG.kernelLength; ++i) {
    ...
    // const input =  InputRingBuffer[0][inputReadIndex];
    // const last =   InputRingBuffer[0][inputReadIndex-CONFIG.kernelLength];
    // const accel = (input - last)/2;
    // if( typeof input !=='undefined' && input>0){
    //   sum += (input * input);
    //   avgAccel += accel;
    // }
    var delay = 12800;
    if(inputReadIndex > delay){
      var delayedIndex = inputReadIndex - delay;
    }else{
      var delayedIndex = CONFIG.ringBufferLength - (delay - inputReadIndex)
    }
    var predelay = inputReadIndex > delay ? InputRingBuffer[0][delayedIndex] * -0.2 : 0;
    OutputRingBuffer[0][outputWriteIndex] = InputRingBuffer[0][inputReadIndex] + predelay;
  ...
  if(sum>0 || avgAccel>0) console.log(sum, avgAccel);
