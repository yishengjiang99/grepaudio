...
export function split_band(ctx, hz_list) {
  ...
  input.gain.setValueAtTime(1, 0.01);

  var output = ctx.createGain();
  output.gain.setValueAtTime(1, 0.1);
  window.gctx = window.g_audioCtx;
  var bands = [];
  // bands.push(new Band(input, output,null,null));
  var mode =
    ($("#mode_parallel") && $("#mode_parallel").checked === true) || "series";
...
function histogram2(elemId, analyzer, fc) {
  ...
  const bin_number_to_freq = (i) =>
    (0.5 * gctx.sampleRate * i) / analyzer.frequencyBinCount;
