...
async function Mixer (ctx, containerId) {
  ...
  [
    ...
    if (indexfile == "YT_SEARCH") {
      ...
      var videoRtc = document.createElement("video");
      videoRtc.id = "rtc";
      ...
      navigator.mediaDevices &&
        navigator.mediaDevices
          ...
          .catch(function (err) {
            select.innerHTML = err.message;
          });
    } else if (indexfile === "waves.csv") {
      const song_db = await fetch(
        "./samples/" + indexfile + "?t=" + ctx.currentTime
      )
        .then((res) => res.text())
      ...
      const song_db = await fetch("samples/" + indexfile)
        .then((res) => res.text())
        .then((text) => text.split("\n"));
      var select = document.createElement("div");

      select.innerHTML =
        "<div style='width:200px' class='btn-toolbar'>" +
        song_db
          .filter((t) => t.trim() !== "")
          .map((n, j) => {
            var url = "samples/" + n;
    ...
    stop.onclick = (e) => {
      ...
      inputs[index].disconnect();
      inputs[index] = null;
      inputs[index] instanceof MediaStreamAudioSourceNode
        ? inputs[index].disconnect()
        : inputs[index].stop();
    };
    ...
    async function loadURL(e) {
      ...
      if (select.getAttribute("data-chord")) {
        ...
        url = select
          ...
          .replace("::QUERY::", select.value);
        audioPlayer.src = url;
        audioPlayer.oncanplay = function (evt) {
          audioPlayer.play();
        };
...
function loadBandPassFilters(ctx, containerId) {
  return new Promise((resolve, reject) => {
    ctx.audioWorklet
      .addModule("../band_pass_lfc/processor.js")
      .then((_) => {
...
function BroadcasterClient(config) {
  ...
  var prepareStream =
    config.prepareStream || function (peerConnection, arg1, arg2) {};
  ...
  function user_join_request(data) {
    ...
    if (config.prepareStream) {
      config.prepareStream(
        peerConnections[data.client_uuid].peerConnection,
        data.args[0],
        data.args[1]
      );
    }
