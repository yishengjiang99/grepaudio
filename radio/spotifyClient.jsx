...
//accounts.spotify.com/authorize?client_id=3993d63f6507434d9ec90cc704b435d9&scope=playlist-read-private%20playlist-read-collaborative%20playlist-modify-public%20user-read-recently-played%20playlist-modify-private%20ugc-image-upload%20user-follow-modify%20user-follow-read%20user-library-read%20user-library-modify%20user-read-private%20user-read-email%20user-top-read%20user-read-playback-state&response_type=token&redirect_uri=http://localhost:3000/playback
const AUTH_URL =
  "https://accounts.spotify.com/authorize?client_id=3993d63f6507434d9ec90cc704b435d9&scope=" +
  scope.join(",") +
...
export const getPlayList = async (token, containerId) => {
  const playlistJson = await fetchAPI("/me/playlists").then((res) =>
    res.json()
  );
...
export const getTracks = async function (token, playlist, containerId) {
  const trackListJson = await fetchAPI(
    ...
    window.onSpotifyWebPlaybackSDKReady = () => {
      ...
      window.webplayer.addListener("player_state_changed", (e) => {
        ...
        if (e.track_window.current_track) {
          ...
          renderNowPlaying(
            e.track_window.current_track.album.images[0].url,
            [],
            e.track_window.current_track.name,
            e.track_window.current_track.artists.map((a) => a.name).join(", ")
          );
        }
        if (e.track_window.next_tracks) {
          $("#nexttracks").innerHTML = e.track_window.next_tracks
